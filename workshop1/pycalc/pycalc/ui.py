...
class PyCalcUI(tk.Frame):
    ...
    def create_widgets(self) -> None:
        ...
        self.op_btns['='] = tk.Button(self, text='=', command=self.on_op_clicked('='))
        self.op_btns['='].grid(row=4, column=2, **opts)

        for i, op in enumerate(Calculator.SUPPORTED_OPERATORS, 1):
            self.op_btns[op] = tk.Button(self, text=op, command=self.on_op_clicked(op))
